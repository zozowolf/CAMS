...
namespace test
{
    public class SQL_command
    {
        ...
        public List<string> GetDate(int idChannel, int idEnregistrement)
        {
            string cn_string = Properties.Settings.Default.DBCAMSConnectionString;
            List<string> dates = new List<string>(); // Initialiser la liste de dates

            using (SqlConnection cn_connection = new SqlConnection(cn_string))
            {
                cn_connection.Open();

                string sql_Text = $"SELECT FORMAT(dateHeure, 'dd/MM/yy') AS Date FROM Mesure WHERE Id = '{idChannel}' AND IdEnregistrement = '{idEnregistrement}'";

                using (SqlCommand cmd = new SqlCommand(sql_Text, cn_connection))
                {
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        // Lire toutes les dates et les ajouter à la liste
                        while (reader.Read())
                        {
        ...
        public List<int> GetHeure(int idChannel, int idEnregistrement)
        {
            string cn_string = Properties.Settings.Default.DBCAMSConnectionString;
            List<int> heures = new List<int>(); // Initialiser la liste des heures

            using (SqlConnection cn_connection = new SqlConnection(cn_string))
            {
                cn_connection.Open();

                string sql_Text = $"SELECT FORMAT(dateHeure, 'HH') AS Heure FROM Mesure WHERE Id = '{idChannel}' AND IdEnregistrement = '{idEnregistrement}'";

                using (SqlCommand cmd = new SqlCommand(sql_Text, cn_connection))
                {
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        // Lire toutes les heures et les ajouter à la liste
                        while (reader.Read())
                        {
                            if (!reader.IsDBNull(reader.GetOrdinal("Heure")))
                            {
                                int heure = Convert.ToInt32(reader["Heure"]);
                                heures.Add(heure);
                            }
                        }
                    }
        ...
        public string GetCapteur(int idChannel)
        {
            string cn_string = Properties.Settings.Default.DBCAMSConnectionString;
            using (SqlConnection cn_connection = new SqlConnection(cn_string))
            {
                ...
                using (SqlCommand cmd = new SqlCommand(sql_Text, cn_connection))
                {
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        ...
                        {
                            capteur = reader["TypeCapteur"].ToString();
                        }
                    }
                }
        ...
        public List<int> GetValeurs(int idChannel, int idEnregistrement)
        {
            string cn_string = Properties.Settings.Default.DBCAMSConnectionString;
            List<int> valeurs = new List<int>(); // Initialiser la liste des valeurs

            using (SqlConnection cn_connection = new SqlConnection(cn_string))
            {
                cn_connection.Open();

                // Modification pour inclure les paramètres dans la requête SQL
                string sql_Text = $"SELECT valeur FROM Mesure WHERE Id = '{idChannel}' AND IdEnregistrement = '{idEnregistrement}'";
                ...
                using (SqlCommand cmd = new SqlCommand(sql_Text, cn_connection))
                {
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        // Lire toutes les valeurs et les ajouter à la liste
                        while (reader.Read())
                        {
